...
public class controlador_cliente implements Interface_clientes{
    ...
    public List<Cliente> BusquedaCliente(int dni) {
        ...
        Connection conn = null;
        try {
            String sql = "select id_cliente,dni,nombre,apellido,telefono from clientes where dni like ?;";
            conn = MySQLConexion.getConexion();
            //prepara una sentencia para ejecutar instrucciones sql
            PreparedStatement st = conn.prepareStatement(sql);
            st.setString(1,dni+"%");
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
                Cliente ap = new Cliente();
                ap.setId_cliente(rs.getInt(1));
                ap.setDni(rs.getInt(2));
                ap.setNombre(rs.getString(3));
                ap.setApellido(rs.getString(4));
                ap.setTelefono(rs.getInt(5));
                lis.add(ap);
            }
        } catch (Exception ax) {
            ax.printStackTrace();
